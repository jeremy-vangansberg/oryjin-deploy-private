...
def main():
    ...
    example_messages = [
        ...
        "Je lance une startup dans le secteur alimentaire avec des produits healthy. Comment créer une campagne de lancement ?",
        "Je veux promouvoir une application mobile de fitness auprès des jeunes urbains de 25-35 ans"
    ]
    
    # Si la conversation n'a pas encore commencé
    if not st.session_state.conversation_started:
        st.markdown("### 💡 Commencez votre conversation")
        st.markdown("**Exemples de messages que vous pouvez envoyer :**")
        
        for i, example in enumerate(example_messages, 1):
            with st.expander(f"📝 Exemple {i}"):
                st.write(example)
        ...
        if st.button("🚀 Démarrer la conversation", disabled=not initial_message.strip()):
            if initial_message.strip():
                ...
                with st.chat_message("assistant"):
                    ...
                    with st.spinner("Initialisation de la conversation..."):
                        try:
                            # Obtenir le client
